...
if __name__ == '__main__':
    ...
    # Fourth layer - fully connected layer (15*5*64) -> (1024)
    W_fc1 = weight_variable([15 * 5 * KERNEL_2_OUT_CHANNEL, FULLY_CONNECTED_1_OUTPUTS])
    b_fc1 = bias_variable([FULLY_CONNECTED_1_OUTPUTS])
    h_pool2_flat = tf.reshape(h_conv2, [-1, 15 * 5 * 64])
    ...
    train_and_save(sess, 90, 120, True, "30_last")
    # sess = restore_model(sess, "30_last")

    values = []
    for i in range(0, 10):
        batch_xs, batch_ys = dc.get_test_data(50, 90, 120)
